...
with tab3:
    ...
    # 詳細版請見 Tab1 大盤預警
    # ══════════════════════════════════════════════════════════════
    st.markdown("<div class='sec-title'>🌡️ 市場溫度計</div>", unsafe_allow_html=True)
    st.markdown(
        "<div class='infobox'>六大核心指標一目了然，回答：現在適合出手嗎？"
        "<br>完整大盤預警詳細版請見 Tab1。</div>",
        unsafe_allow_html=True
    )
    try:
        _t3_status, _t3_info = get_system_risk_status()
        _t3_vix  = get_vix()
        _t3_mg   = get_total_margin_balance()
        _t3_mg_b = _t3_mg["balance"] if _t3_mg else 0
        _t3_tx   = _t3_info.get("tx_net", 0)
        _t3_rt   = _t3_info.get("mtx_retail", 0)
        _t3_twii = get_index_bias20("^TWII")
        _t3_sox  = get_index_bias20("^SOX")
        _t3_tx_s  = "🟢" if _t3_tx <= -25000 else "🔴" if _t3_tx > -10000 else "🟡"
        _t3_rt_s  = "🟢" if _t3_rt < 0 else "🔴" if _t3_rt > 12000 else "🟡"
        _t3_vx_s  = "🟢" if (_t3_vix or 99) < 20 else "🔴" if (_t3_vix or 0) > 30 else "🟡"
        _t3_mg_s  = "🟢" if _t3_mg_b < 4500 else "🔴" if _t3_mg_b >= 5000 else "🟡"
        _t3_tw_s  = ("🔴" if _t3_twii and _t3_twii["bias_20"] >= 8 else "🟢" if _t3_twii and _t3_twii["bias_20"] <= -8 else "🟡")
        _t3_sx_s  = ("🔴" if _t3_sox and _t3_sox["bias_20"] >= 8 else "🟢" if _t3_sox and _t3_sox["bias_20"] <= -8 else "🟡")
        _t3_tw_val = f"{_t3_twii['bias_20']:+.1f}%" if _t3_twii else "—"
        _t3_sx_val = f"{_t3_sox['bias_20']:+.1f}%" if _t3_sox else "—"
        _t3_all = [_t3_tx_s, _t3_rt_s, _t3_vx_s, _t3_mg_s, _t3_tw_s, _t3_sx_s]
        _t3_red = _t3_all.count("🔴")
        _t3_grn = _t3_all.count("🟢")
        if _t3_red >= 3:   _t3_env, _t3_ec = "🔴 縮手防守", "#ff4444"
        elif _t3_grn >= 4: _t3_env, _t3_ec = "🟢 適合布局", "#00cc66"
        else:              _t3_env, _t3_ec = "🟡 謹慎觀望", "#fbbf24"
        st.markdown(
            f"<div style='border:2px solid {_t3_ec};border-radius:10px;"
            f"padding:14px 18px;margin:12px 0;background:rgba(255,255,255,0.02);'>"  
            f"<div style='font-size:1.2rem;font-weight:700;color:{_t3_ec};"
            f"margin-bottom:10px;'>{_t3_env}</div>"
            f"<div style='display:flex;gap:20px;flex-wrap:wrap;font-size:.88rem;"
            f"margin-bottom:10px;'>"
            f"<span>{_t3_tx_s} 外資大台 {_t3_tx:+,}口</span>"
            f"<span>{_t3_rt_s} 小台散戶 {_t3_rt:+,}口</span>"
            f"<span>{_t3_vx_s} VIX {f'{_t3_vix:.1f}' if _t3_vix else '—'}</span>"
            f"<span>{_t3_mg_s} 融資水位 {f'{_t3_mg_b:,.0f}億' if _t3_mg_b else '—'}</span>"
            f"<span>{_t3_tw_s} 台股月乖離 {_t3_tw_val}</span>"
            f"<span>{_t3_sx_s} 費半月乖離 {_t3_sx_val}</span>"
            f"</div></div>",
            unsafe_allow_html=True
        )
    except Exception:
