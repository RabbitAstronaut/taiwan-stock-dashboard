...
with tab9:
    ...
    st.markdown(
        "<div class='infobox'>未來三週、現金殖利率 4% 以上標的，"
        "套用精兵評分排序。殖利率高不等於好標的，精兵總分才是關鍵。</div>",
        unsafe_allow_html=True
    )
    ...
    def fetch_twse_dividend():
        """
        從 TWSE 抓取除權息預告資料（TWT49U）。
        回傳 DataFrame，欄位：stock_id, name, ex_date, cash_div, stock_div
        失敗時回傳空 DataFrame。
        """
        import requests as _req
        ...
        # TWSE 除權息預告（上市）
        _url = "https://www.twse.com.tw/rwd/zh/exRight/TWT49U?response=json"
        _rows = []
        try:
            ...
            if _r.status_code == 200:
                ...
                for _row in _data.get("data", []):
                    ...
                    # 欄位對應：股票代號、名稱、除息日期、現金股利、股票股利
                    _sid   = str(_d.get("股票代號", _d.get("Code", ""))).strip()
                    _name  = str(_d.get("股票名稱", _d.get("Name", ""))).strip()
                    _edate = str(_d.get("除息日期", _d.get("除權息日期", ""))).strip()
                    _cash  = str(_d.get("現金股利", _d.get("息值(元)", "0"))).strip()
                    _stock = str(_d.get("股票股利", _d.get("權值(元)", "0"))).strip()
                    if _sid:
                        _rows.append({
        ...
        try:
            _r2 = _req.get(_url_otc, headers=_headers, timeout=10)
            if _r2.status_code == 200:
                _data2 = _r2.json()
                for _row in _data2.get("aaData", []):
                    # OTC格式：[代號, 名稱, 除息日, 現金股利, 股票股利, ...]
                    if len(_row) >= 5:
                        _rows.append({
        ...
        # 清理股票代號（不含前導零問題，台股代號4碼）
        df["stock_id"] = df["stock_id"].astype(str).str.strip().str.replace(r'\.0$', '', regex=True)

        # 清理現金股利
        def _parse_num(s):
            try:
                return float(str(s).replace(",", "").replace("%", "").replace("--", "0").strip() or 0)
            except Exception:
                return None
        ...
        # 清理日期（民國年 → 西元年）
        def _parse_twse_date(s):
            s = str(s).strip()
            if not s or s in ("--", "N/A", ""):
                return None
            try:
                # 民國格式 113/07/15 → 2024/07/15
                parts = s.replace("-", "/").split("/")
                if len(parts) == 3 and int(parts[0]) < 200:
                    y = int(parts[0]) + 1911
                    return _pd.Timestamp(f"{y}/{parts[1]}/{parts[2]}")
                return _pd.to_datetime(s, errors="coerce")
            except Exception:
                return None

        df["ex_date"] = df["ex_date_str"].apply(_parse_twse_date)
    ...
    # ── 計算日期範圍
    _today = datetime.now(ZoneInfo("Asia/Taipei")).date()
    _end_date = _today + timedelta(days=DIVIDEND_LOOKAHEAD_DAYS)
    _update_time = datetime.now(ZoneInfo("Asia/Taipei")).strftime("%Y/%m/%d %H:%M")
    ...
    if df_div.empty:
        ...
        st.markdown(
            ...
            f"符合條件：<b style='color:#00e676;'>{len(_qualified)} 檔</b>　"
            f"資料不足：<b style='color:#ff9800;'>{len(_missing)} 檔</b>"
            f"</div>",
            unsafe_allow_html=True
        )
        ...
        if not _qualified:
            ...
            st.download_button(
                "⬇️ 下載榜單 CSV", data=_csv9.getvalue().encode("utf-8-sig"),
                file_name=f"除權息精兵榜_{_today.strftime('%Y%m%d')}.csv",
                mime="text/csv", key="dl_div9"
            )

        # ── 資料不足清單
        if _missing:
            with st.expander(f"⚠️ 資料不足（共 {len(_missing)} 檔）", expanded=False):
                for _m in _missing:
                    st.markdown(
                        f"<div style='font-size:.82rem;color:#ff9800;padding:3px 0;'>"
                        f"{_m['stock_id']} {_m['name']}　"
                        f"除息 {pd.Timestamp(_m['ex_date']).strftime('%m/%d')}　"
                        f"現金股利 {_m['cash_div']:.2f}　"
                        f"缺少：{_m['缺少']}</div>",
                        unsafe_allow_html=True
                    )
